...
# How to run
По умолчанию запустится в браузере chrome и на английском языке:
```
pytest test_items.py
```
Для выбора языка нужно использовать ключ `--language=es`
```
pytest --language=es test_items.py
```
Для выбора браузера нужно использовать ключ `--browser_name=firefox`
```
pytest test_items.py -s --browser_name=firefox
```